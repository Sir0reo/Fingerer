...
class FingererApp:
    def __init__(self, root):
        ...
        # Click sensitivity slider — higher = left click registers more easily.
        tk.Label(root, text="Click sensitivity").pack()
        self.click_var = tk.IntVar(value=DEFAULT_CLICK_SENS)
        tk.Scale(
            root, from_=CLICK_SENS_MIN, to=CLICK_SENS_MAX, orient="horizontal",
            length=220, variable=self.click_var, command=self.on_click_sens,
        ).pack(padx=20, pady=(0, 10))
    ...
    def on_click_sens(self, value):
        self.tracker.set_click_sensitivity(int(float(value)))
